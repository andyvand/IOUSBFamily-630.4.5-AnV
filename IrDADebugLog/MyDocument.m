...
- (void)updateTotal {
    [totalField setObjectValue: [NSNumber numberWithInteger:[[self logItems] count]]];
}
...
- (void)createNewLogItem:(id)sender {
    ...
    [self updateViews];
    rowIndex = [[NSIndexSet alloc] initWithIndex:(NSUInteger)row];
    [tableview selectRowIndexes:rowIndex byExtendingSelection:NO];
    [rowIndex release];
...
- (int)numberOfRowsInTableView:(NSTableView *)tv {
    [self updateTotal];
    return (int)[[self logItems] count];
}
    ...
    row: (int)row
{
    ...
    */
	...
	if ([[tc identifier] isEqualTo: @"time"] ){
		...
		snprintf( newTime, sizeof(newTime), "%4ld.%06ld", (long)secs, (long)usecs);
		return [NSString stringWithUTF8String:newTime];
	}
	...
	if ([[tc identifier] isEqualTo: @"second"] | [[tc identifier] isEqualTo: @"first"]){
		...
		return [NSString stringWithUTF8String:hexValue];
	}
