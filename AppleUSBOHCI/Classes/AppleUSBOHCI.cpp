...
IOReturn
AppleUSBOHCI::UIMAllocDev(UInt8 devAddr, UInt8 devSpd)
{
...
IOReturn
AppleUSBOHCI::UIMPassEPDesAddr(UInt8 address, struct Endpoint *pEP)
{
...
AppleUSBOHCI::init(OSDictionary * propTable)
{
	...
	OSDictionary *usbOHCIDict;
	usbOHCIDict = OSDynamicCast(OSDictionary, propTable);
	if (OSDynamicCast(OSDictionary, usbOHCIDict) != NULL) {
		if (usbOHCIDict->getObject("SleepFix") != NULL)
		{
			OSBoolean * sleepFix;
			sleepFix= OSDynamicCast(OSBoolean, usbOHCIDict->getObject("SleepFix"));
...
void 
AppleUSBOHCI::showRegisters(UInt32 __unused level, const char * __unused s)
{
