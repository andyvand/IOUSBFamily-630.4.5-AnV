...
void TIASService::free()
{
	...
	TIASClass* theClass = (TIASClass*)this->At((ArrayIndex)index);
	theClass->release();
    }
...
void TIASClass::free()
{
    ...
    for (index = 0; index < this->GetArraySize(); index++) {
	TIASAttribute* theAttr = (TIASAttribute*)this->At((ArrayIndex)index);
	theAttr->release();
    }
...
void TIASAttribute::free()
{
    ...
    // removing them because I'm depending on the list destructor to do that for me.
    for (index = 0; index < this->GetArraySize(); index++) {
	TIASElement* theElement = (TIASElement*)this->At((ArrayIndex)index);
	theElement->release();
    }
    super::free();
...
Boolean TIASNamedList::Init(const UChar *theName)
{
    ...
    fNameLen = (int)(strlen((const char*)theName) + 1);
    if (fNameLen) {
...
void TIASElement::AddInfoToBuffer(CBuffer* buffer)
{
	...
	*pHdr++ = (UByte)length;    // Length of string
    ...
    // Put out the header
    buffer->Putn(header, (Size)(pHdr - header));
