	...
	case cmdWrite:
			{
			    ...
			    if (result)
                    *result = (UInt32)obj->fIrComm->Write(buf, (UInt32)length);
			}
			break;
	...
	case cmdReturnCredit:
			{
			    ...
			    if (obj->fIrComm)
                    obj->fIrComm->ReturnCredit((UInt32)byte_count);
			}
			break;
	...
	case cmdReadComplete:
			{
			    ...
			    uintptr_t  length  = (uintptr_t)arg3;
			    if (obj->fIrDA) {
				obj->fIrDA->ReadComplete(buf, (UInt32)length);
				obj->fIrDA->RunQueue();
			    }
			}
			break;
