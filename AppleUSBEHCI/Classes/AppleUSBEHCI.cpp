...
IOReturn
AppleUSBEHCI::UIMAllocDev(UInt8 devAddr, UInt8 devSpd)
{
#pragma unused (devAddr)
#pragma unused (devSpd)
	return kIOReturnSuccess;
}

IOReturn
AppleUSBEHCI::UIMPassEPDesAddr(UInt8 address, struct Endpoint *pEP)
{
...
AppleUSBEHCI::init(OSDictionary * propTable)
{
	...
	if (OSDynamicCast(OSDictionary, usbEHCIDict) != NULL) {
		...
		if (usbEHCIDict->getObject("SleepFix") != NULL)
		{
			OSBoolean * sleepFix;
			sleepFix= OSDynamicCast(OSBoolean, usbEHCIDict->getObject("SleepFix"));
...
void AppleUSBEHCI::showRegisters(UInt32 __unused level, const char * __unused s)
{
    int				i;
...
UInt32
AppleUSBEHCI::GetErrataBits(UInt16 vendorID, UInt16 deviceID, UInt16 revisionID ){
  return IOUSBControllerV3::GetErrataBits(vendorID, deviceID, revisionID );
}
...
AppleUSBEHCI::AcquireOSOwnership(void)
{
  ...
  bool			isOwnershipConflict;
	...
	USBLog(3, "Attempting to get EHCI Controller from BIOS");
  ...
  if ((data & 0xFF) == kEHCI_USBLEGSUP_ID)
  {
		...
		{
			USBLog(1, "EHCI - Ownership conflict - attempting soft reset ...");
			USBLog(1, "EHCI - toggle OS Ownership to 0");
    ...
    for (int i = 0; i < 25; i++)
    {
      ...
      // Soft reset has failed. Assume SMI being ignored
			...
			USBLog(1, "EHCI - Ownership conflict - attempting hard reset ...");
			USBLog(1, "EHCI - Force BIOS Ownership to 0");
