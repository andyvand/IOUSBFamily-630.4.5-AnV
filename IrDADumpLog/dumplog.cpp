...
#define _KERN_TASK_H_ 1
#define _LIBKERN_LIBKERN_H_ 1
#define _MACH_H_ 1
...
#define PuntErr(x)  { fprintf(stderr, "%s: %s\n", argv[0], x); return 1; }
...
DumpLocalLog()
{
    ...
    netif = getInterfaceWithName(masterPort, (char *)"AppleIrDA");
    if (netif) {
	//printf("netif=0x%x\n", netif);
	...
	if (kr == kIOReturnSuccess) {
	    ...
	    inbuf[0] = (UInt32)(UInt64)&bigbuffer[0];
...
{
    return IOServiceOpen(obj, (task_port_t)mach_task_self(), 123, con);
}
	    ...
	    void *outputData, size_t *outputDataSize)
{
    ...
    err = IOConnectCallStructMethod(
			(mach_port_t)con,
			0,                                  /* method index */
			(const void *) command,                   /* input[] */
			inputDataSize+sizeof(unsigned char),/* inputCount */
			(void *) outputData,                /* output */
...
{ 
    ...
    rc = sscanf(argv[2], "0x%lx", (unsigned long *)&infoaddr);
    ...
    ok = ReadLog((void *)(UInt64)infoaddr);
...
DoConnect(UInt16 localport)
{
    ...
    length = (int)(sizeof(kdp_connect_req_t) + strlen(msg) + 1);
...
ReadLog(void *addr)
{
    ...
    printf("hdr  at 0x%llx, size %d\n", (UInt64)info.hdr,       (int)info.hdrSize);
    printf("log  at 0x%llx, size %d\n", (UInt64)info.eventLog,  (int)info.eventLogSize);
    printf("msgs at 0x%llx, size %d\n", (UInt64)info.msgBuffer, (int)info.msgBufferSize);
...
{   
    ...
    time(&now);
    lt = localtime(&now);
    snprintf(filename, sizeof(filename), "log.%d.%d.%02d%02d.%02d",
...
Boolean CheckLog(IrDALogHdr *obj)
{
	...
	printf("gIrDALog = 0x%llx\n", (UInt64)obj);
	printf("buffer   = 0x%llx\n", (UInt64)obj->fEventBuffer);
	printf("index    = %ld\n",   (long)obj->fEventIndex);
	printf("prtindex = %ld\n",   (long)obj->fPrintIndex);
	printf("#entries = %ld\n",   (long)obj->fNumEntries);
	printf("traceon  = %d\n",   obj->fTracingOn);
	printf("wrapped  = %d\n",   obj->fWrapped);
	return false;
    }
...
{   
	...
	if (eventPtr->timeStamp) {          // if log entry is timestamped
	    ...
	    len = snprintf( buffer, sizeof(buffer), "%4ld.%06ld [%6ld]  D: %04hx,%04hx  %s",
	    ...
	    len = snprintf( buffer, sizeof(buffer), "   x.x      [      ]  D: %04hx,%04hx  %s",
						    //secs, usecs,
						    //deltaTime,
						    eventPtr->data1,
						    eventPtr->data2,
