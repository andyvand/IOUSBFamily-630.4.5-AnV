...
bool check_crc32( unsigned char *buf, int len )
{
    ...
    crc = (UInt32)update_crc32((UInt32)crc, buf, len);
...
UInt16 AppleSCCIrDA::SetSpeed( UInt32 brate )
{
	...
	bool rc;
	fSpeedChanging = true;
	rc = thread_call_enter1(speed_change_thread_call, (thread_call_param_t)(UInt64)command);
...
IOReturn AppleSCCIrDA::requestEvent( UInt32 event, UInt32 *data )
{
	...
	switch ( event )
	{
		...
		*data = (UInt32)tval2long( port->CharLatInterval )/ 1000;
		break;
	    
	    case PD_E_DATA_LATENCY:
		ELG( 0, event, 'ReqE', "requestEvent - PD_E_DATA_LATENCY" );
		*data = (UInt32)tval2long( port->DataLatInterval )/ 1000;
		break;
...
IOReturn AppleSCCIrDA::enqueueData( UInt8 *buffer, UInt32 size, UInt32 *count, bool sleep )
{
    ...
    *count = (UInt32)AddtoQueue( &port->TX, buffer, size );
    CheckQueues( port );
...
IOReturn AppleSCCIrDA::dequeueData( UInt8 *buffer, UInt32 size, UInt32 *count, UInt32 min )
{
    ...
    *count = (UInt32)RemovefromQueue( &port->RX, buffer, size );
    if (fIrDA)
	...
	count_read = (UInt32)RemovefromQueue( &port->RX, buffer + *count, (size - *count) );
	if (fIrDA)
...
IOReturn AppleSCCIrDA::StartTransmit( UInt32 control_length, UInt8 *control_buffer, UInt32 data_length, UInt8 *data_buffer )
{
    ...
    thread_call_enter1(tx_thread_call, (thread_call_param_t)(UInt64)count);     // send it out and wait for it in another workloop
...
{
    AppleSCCIrDA *obj;
    UInt32 count = (UInt32)(UInt64)param1;
    UInt32 tCount;
    UInt32  waitBits;
    IOReturn rtn;
