...
IOReturn AppleUSBIrDADriver::enqueueData( UInt8 *buffer, UInt32 size, UInt32 *count, bool sleep, void *refCon )
{
    ...
    *count = (UInt32)AddtoQueue( &port->TX, buffer, (size_t)size );
    CheckQueues( port );
...
IOReturn AppleUSBIrDADriver::dequeueData( UInt8 *buffer, UInt32 size, UInt32 *count, UInt32 min, void *refCon )
{
    ...
    *count = (UInt32)RemovefromQueue( &port->RX, buffer, (size_t)size );
    if (fIrDA)
	...
	count_read = (UInt32)RemovefromQueue( &port->RX, buffer + *count, (size_t)(size - *count) );
	if (fIrDA)
