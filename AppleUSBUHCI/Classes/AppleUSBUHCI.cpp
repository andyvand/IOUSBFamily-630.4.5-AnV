...
IOReturn
AppleUSBUHCI::UIMAllocDev(UInt8 devAddr, UInt8 devSpd)
{
...
IOReturn
AppleUSBUHCI::UIMPassEPDesAddr(UInt8 address, struct Endpoint *pEP)
{
...
AppleUSBUHCI::init(OSDictionary * propTable)
{
	...
	OSDictionary *usbUHCIDict;
	usbUHCIDict = OSDynamicCast(OSDictionary, propTable);
	if (OSDynamicCast(OSDictionary, usbUHCIDict) != NULL) {
		if (usbUHCIDict->getObject("SleepFix") != NULL)
		{
			OSBoolean * sleepFix;
			sleepFix= OSDynamicCast(OSBoolean, usbUHCIDict->getObject("SleepFix"));
...
void 
AppleUSBUHCI::showRegisters(UInt32 __unused level, const char * __unused s)
{
