...
//const char ehciMuxedPorts[kMaxHCPortMethods][kHCPortMethodNameLen] = {"EHCA","EHCB","EHCC","EHCD"};
...
protected:
    ...
    static bool                             PrimaryInterruptFilterSingleInt(OSObject *owner, IOFilterInterruptEventSource *source);
    ...
    // AnV - Legacy support
    volatile UInt32                        *_xUSBLegSup;	// Added for USB3 legacy support
    ...
    // Default value of SleepFix key
    bool									_sleepFix;

    // Default value of UseSingleInt key
    bool                                    _useSingleInt;

    // Default value of UseLegacyInt key
    bool                                    _useLegacyInt;

    // Default value of ResetControllerFix key
    bool                                    _resetControllerFix;

    // Default value of the NoSleepForced key
    bool                                    _noSleepForced;
...
//	UInt32 GetErrataBits(UInt16 vendorID, UInt16 deviceID, UInt16 revisionID);
  ...
  bool IsPortMuxed(IORegistryEntry * device, UInt16 port, UInt32  locationID, char* muxedPort);
...
public:
    ...
    virtual IOReturn    CompleteSuspendOnAllPorts(void);
    virtual IOReturn    CommandStop(void);
    virtual uint16_t    PortNumberCanonicalToProtocol(uint16_t canonical, uint8_t* pProtocol);
    virtual IOUSBHubPolicyMaker*    GetHubForProtocol(uint8_t protocol);
    virtual bool        RHCheckForPortResume(uint16_t port, uint8_t protocol, uint32_t havePortSC);
    virtual void        SantizePortsAfterPowerLoss(void);
    virtual IOReturn    InitializePorts(void);
    virtual void        TakeOwnershipFromBios(void);
    virtual IOReturn    XHCIHandshake(volatile UInt32 *pReg, UInt32 test_mask, uint32_t test_target, int32_t msec);
    virtual void        SetPropsForBookkeeping(void);
