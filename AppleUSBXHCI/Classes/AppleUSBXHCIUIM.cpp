...
#include "../../IOUSBFamily/Headers/IOUSBPriv.h"
...
#ifndef kUSB_EPDesc_MaxMPS
#define kUSB_EPDesc_MaxMPS 1024
#endif

#ifndef kAppleUSBSSIsocContinuousFrame
...
#endif
#ifndef kIOUSBMessageMuxFromXHCIToEHCI
...
// AnV: Added generic errata and expanded errata list for more generic parameters
static ErrataListEntry  errataForcedNoMSI = {0, 0, 0, 0xffff, kXHCIErrata_NoMSI};
static ErrataListEntry  errataForced = {0, 0, 0, 0xffff, kXHCIErrata_Etron7052};
static ErrataListEntry  errataList[] = {
    
    {0x1033, /*0x0194*/0, 0, 0xffff,	kXHCIErrata_NEC},	// NEC XHCI, check firmware
    {0x8086, /*0x1e31*/0, 0, 0xffff,	kXHCIErrataPPT | kXHCIErrataPPTMux | kXHCIErrata_EnableAutoCompliance | /*kErrataSWAssistXHCIIdle |*/ kXHCIErrata_ParkRing},	// Intel Panther Point Compatible
    {0x1b21, 0, 0, 0xffff,	kXHCIErrata_ASMedia},   // ASMedia XHCI
    {0x1b73, 0x1000, 0, 0xffff,	kXHCIErrata_NoMSI},	// Fresco Logic XHCI
	{0x1b73, 0x1100, 0, /*16*/0xffff, kXHCIErrata_FL1100_Ax},	// Fresco Logic FL1100-Ax
	{0x1b73, 0,	0, 0xffff, kXHCIErrata_FrescoLogic},// Fresco Logic
	{0x1b6f, /*0x7052*/0, /*1*/0, 0xffff, kXHCIErrata_Etron7052}, // Etron all
	{0x1106, 0x0, 0, 0xffff,	kXHCIErrata_NoMSI},	// VIA VL800
};
/*
...
UInt32 AppleUSBXHCI::GetErrataBits(UInt16 vendorID, UInt16 deviceID, UInt16 revisionID)
{
    ...
    /* AnV: Set use of legacy interrupts */
    if (_useLegacyInt == true)
    {
        errata |= errataForcedNoMSI.errata;
    }

    /* AnV: If no errata matches, set the forced one */
    if (errata == 0)
    {
        errata |= errataForced.errata;
    }
...
void AppleUSBXHCI::PrintInterrupter(int __unused level, int __unused IRQ, const char * __unused s)
{
...
// WaitForCMD IRQ:0 - IMAN: 00000002 IMOD: 000000a0 ERDP: 7c002300
...
void AppleUSBXHCI::PrintTRB(int __unused level, TRB * __unused trb, const char * __unused s, UInt32 offsC)
{
...
SInt32 AppleUSBXHCI::WaitForCMD(TRB *t, int command, CMDComplete callBackF)
{
  ...
  if((Read32Reg(&_pXHCIRegisters->USBSTS) & kXHCIHSEBit) != 0)
  {
    if(!_HSEReported)
    {
      USBError(1, "AppleUSBXHCI[%p]::WaitForCMD - HSE bit set:%x (1)", this, (unsigned int)Read32Reg(&_pXHCIRegisters->USBSTS));
    }
    _HSEReported = true;
  }
...
/*
 USBF:	1. 82	AppleUSBXHCI[0xffffff80d8313000]::WaitForCMD (Configure Endpoint Command) - Command failed:-1017 (num interrupts: 35, num primary: 35, inactive:0, unavailable:0, is controller available:1)
 USBF:	1. 89	AppleUSBXHCI[0xffffff80d8313000]::PrintInterrupter WaitForCMD IRQ:0 - IMAN: 00000002 IMOD: 000000a0 ERDP: 7c002300
 USBF:	1. 95	AppleUSBXHCI[0xffffff80d8313000]::ClearEndpoint - configure endpoint failed:-1017

 -1017 == kXHCITRB_CC_CtxParamErr
 */
...
//  USBF:	1. 89	AppleUSBXHCI[0xffffff80d8313000]::PrintInterrupter WaitForCMD IRQ:0 - IMAN: 00000002 IMOD: 000000a0 ERDP: 7c002300
//  USBF:	1. 95	AppleUSBXHCI[0xffffff80d8313000]::ClearEndpoint - configure endpoint failed:-1017
//  USBF:	1.101	AppleUSBXHCI[0xffffff80d8313000]::ClearEndpoint - Input Context 0
//  USBF:	1.106	AppleUSBXHCI[0xffffff80d8313000]::ClearEndpoint - Input Context 1
//  USBF:	1.112	AppleUSBXHCI[0xffffff80d8313000]::ClearEndpoint - Input Context X
...
bool AppleUSBXHCI::DoCMDCompletion(TRB nextEvent, UInt16 __unused eventIndex)
{
    USBPhysicalAddress64 phys;
    CMDComplete c;
	int index;
...
#pragma unused(IRQ)
    ...
    if((Read32Reg(&_pXHCIRegisters->USBSTS)& kXHCIHSEBit) != 0)
    {
        if(!_HSEReported)
        {
            USBError(1, "AppleUSBXHCI[%p]::PollEventRing2 - HSE bit set:%x (1)", this, (unsigned int)Read32Reg(&_pXHCIRegisters->USBSTS));
        }
        _HSEReported = true;
    }
    ...
    if((Read32Reg(&_pXHCIRegisters->USBSTS)& kXHCIHSEBit) != 0)
    {
        if(!_HSEReported)
        {
            USBError(1, "AppleUSBXHCI[%p]::PollEventRing2 - HSE bit set:%x (3)", this, (unsigned int)Read32Reg(&_pXHCIRegisters->USBSTS));
        }
        _HSEReported = true;
    }
...
#pragma unused(source)
    ...
    result = controller->FilterInterrupt(kPrimaryInterrupter);
...
IOReturn AppleUSBXHCI::UIMInitialize(IOService * provider)
{
    ...
    do {
        ...
        /* AnV: Modded to enable all XHCI controllers */
        ...
        /* AnV: Allow all HCIVersions */
        HCIVersion = Read16Reg(&_pXHCICapRegisters->HCIVersion);
      USBError(2, "AppleUSBXHCI[%p]::UIMInitialize - HCIVersion:%x", this, (unsigned int)HCIVersion);
        if (HCIVersion < kXHCIVersion0100)
        {
            HCIVersion = kXHCIVersion0100;
        }
      ...
      USBLog(2, "AppleUSBXHCI[%p]::UIMInitialize - _MaxInterrupters:%d", this, (int)_MaxInterrupters);
		...
		
		{
			...
			if(msi_interrupt_found && ( (_errataBits & kXHCIErrata_NoMSI) == 0))
			{
                ...
                if (_useSingleInt == false)
                {
                    ...
                    _filterInterruptSource = IOFilterInterruptEventSource::filterInterruptEventSource(this,
                                                                                                      AppleUSBXHCI::InterruptHandler,
                                                                                                      AppleUSBXHCI::PrimaryInterruptFilterSingleInt,
                                                                                                      _device, msi_interrupt_index );
                }
			}
			else
        ...
        /* AnV: Increased to 1000 milliseconds limit */
		if (i >= 1000)
		{
            USBError(1, "AppleUSBXHCI[%p]::UIMInitialize - could not get chip to halt within 1000 ms",  this);
        ...
        // AnV - Take ownership from BIOS for legacy
...
/* AnV - Handshake function */
IOReturn AppleUSBXHCI::XHCIHandshake(volatile UInt32 *pReg, UInt32 test_mask, uint32_t test_target, int32_t msec)
{
    UInt32 reg;
    SInt32 count;
...
/* AnV - Legacy support */
void AppleUSBXHCI::TakeOwnershipFromBios(void)
{
	UInt32 v;
	IOReturn rc;
	...
	if (!_xUSBLegSup)
		return;
	v = Read32Reg(_xUSBLegSup);
	if (v & XHCI_HC_BIOS_OWNED)
    {
		Write32Reg(_xUSBLegSup, v | XHCI_HC_OS_OWNED);
		rc = XHCIHandshake(_xUSBLegSup, XHCI_HC_BIOS_OWNED, 0U, 100);
...
void AppleUSBXHCI::DecodeExtendedCapability()
{
	...
	do{
		...
		{
            case kXHCILegacySupport:
                _xUSBLegSup = (volatile UInt32 *)CurrentXECPBase;
                break;
...
IOReturn AppleUSBXHCI::AddressDevice(UInt32 slotID, UInt16 maxPacketSize, bool setAddr, UInt8 speed, int highSpeedHubSlot, int highSpeedPort)
{
    ...
    if (_useSingleInt == false)
    {
        ...
        SetSlCtxInterrupter(inputContext, kPrimaryInterrupter);
    }
...
//::AddressDevice - Address device failed:-1017
...
#define DEBUG_CDBS (0)
	...
	do
  {
    ...
    if (_useSingleInt == false)
    {
      ...
      newTRB->offs8 = HostToUSBLong(kPrimaryInterrupter << kXHCITRB_InterrupterTarget_Shift);
    }
...
AppleUSBXHCI::CreateInterruptEndpoint(short				functionAddress,
									  ...
									  short				pollingRate,
									  USBDeviceAddress  __unused highSpeedHub,
									  int				__unused highSpeedPort,
...
AppleUSBXHCI::init(OSDictionary * propTable)
{
    ...
    _useSingleInt = false;
    _useLegacyInt = false;
    _resetControllerFix = false;
    _noSleepForced = false;

    // --> PCGen Patch 1
	// get custom keys
	OSDictionary *usbXHCIDict;
	usbXHCIDict = OSDynamicCast(OSDictionary, propTable);
	if (OSDynamicCast(OSDictionary, usbXHCIDict) != NULL) {
		if (usbXHCIDict->getObject("SleepFix") != NULL)
		{
			OSBoolean * sleepFix;
			sleepFix= OSDynamicCast(OSBoolean, usbXHCIDict->getObject("SleepFix"));
			if (sleepFix->isTrue()) _sleepFix = true;
		}

        /* AnV - Only one interrupter fix */
		if (usbXHCIDict->getObject("UseSingleInt") != NULL)
		{
			OSBoolean * singleInt;
			singleInt= OSDynamicCast(OSBoolean, usbXHCIDict->getObject("UseSingleInt"));
			if (singleInt->isTrue()) _useSingleInt = true;
		}

        /* AnV - Use legacy interrupts fix */
        if (usbXHCIDict->getObject("UseLegacyInt") != NULL)
		{
			OSBoolean * legacyInt;
			legacyInt= OSDynamicCast(OSBoolean, usbXHCIDict->getObject("UseLegacyInt"));
			if (legacyInt->isTrue()) _useLegacyInt = true;
		}

        /* AnV - Proper support for PM fix */
        if (usbXHCIDict->getObject("ResetControllerFix") != NULL)
		{
			OSBoolean * resetController;
			resetController= OSDynamicCast(OSBoolean, usbXHCIDict->getObject("ResetControllerFix"));
			if (resetController->isTrue()) _resetControllerFix = true;
		}

        /* AnV - Force sleep support off */
        if (usbXHCIDict->getObject("NoSleepForced") != NULL)
		{
			OSBoolean * SleepIsForced;
			SleepIsForced= OSDynamicCast(OSBoolean, usbXHCIDict->getObject("NoSleepForced"));
			if (SleepIsForced->isTrue()) _noSleepForced = true;
		}
    }
	// <-- End PCGen Patch 1
...
AppleUSBXHCI::HCSelect ( UInt8 port, UInt8 controllerType )
{
  ...
  if( controllerType == kControllerXHCI ) {
    ...
    USB3SSEN &= USB3PRM;
    USB3SSEN |= (1 << (port - 1));
    _device->configWrite32(kXHCI_XUSB3_PSSEN, USB3SSEN);
    ...
    HCSEL &= HCSELM;
    HCSEL |= (1 << (port - 1));
    _device->configWrite32(kXHCI_XUSB2PR, HCSEL);
    Status = kIOReturnSuccess;
  }
...
//================================================================================================
//
// Slice: why we use kMaxHCPortMethods if we know _rootHubNumPorts?
bool 
AppleUSBXHCI::DiscoverMuxedPorts( )
{
	...
	UInt8   port;
  UInt32  locationID = 0;
  UInt8   totalMuxedPorts = _rootHubNumPorts; 
  
 

  if (!_muxedPorts) {
    if (_rootHubDeviceSS && ((_errataBits & kXHCIErrataPPTMux) != 0) )
    {
      ...
      if ( locationIDProperty )
      {
        ...
        if (_acpiDevice)
        {
          for(port = 1; port < kMaxHCPortMethods; port++)
          {
            ...
            {
              USBLog(3, "AppleUSBXHCI[%p]::DiscoverMuxedPorts port %d method %s", this, port, ehciMuxedPorts[port-1]);
              totalMuxedPorts++;
            }
          }
        }
      ...
      for(port = 1; port < kMaxHCPortMethods; port++)
      {
        ...
        if( IsPortMuxed(_device, port, locationID, ehciMuxedPorts[port-1]) )
        {
          USBLog(3, "AppleUSBXHCI[%p]::DiscoverMuxedPorts-2 port %d method %s", this, port, ehciMuxedPorts[port-1]);
          totalMuxedPorts++;
        }
      }
    }
...
bool
AppleUSBXHCI::IsPortMuxed(IORegistryEntry *device, UInt16 port, UInt32  locationID, char* muxedPort)
{
#pragma unused (device)
//#pragma unused (port)
#pragma unused (locationID)
#pragma unused (muxedPort)
  // if Method (EHCA, ... is present then the port is muxed
  // we will do this allways
  muxedPort[0] = 'E';
  muxedPort[1] = 'H';
  muxedPort[2] = 'C';
  muxedPort[3] = 'A' + port - 1;
  return true;
}
...
#if 0  
 if (!_acpiDevice) {
    USBLog(3, "AppleUSBXHCI[%p]::HasMuxedPorts true by default", this);
    return true; //without acpi we will suppose all ports are muxed! ;)
  }
...
AppleUSBXHCI::EnableXHCIPorts()
{
	...
	if (HCSEL == (UInt32)-1)
	{
    USBLog(3, "AppleUSBXHCI[%p]::EnableXHCIPorts not Available", this);
