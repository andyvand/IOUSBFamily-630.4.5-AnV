...
CollectTraceXHCI ( kd_buf tracepoint ) 
{
	...
	switch ( type )
	{
		...
		case USB_XHCI_TRACE(kTPXHCIRead32Reg):
			{
				log (info, "XHCI", "Read32Reg", parg1, "Register: 0x%lx, Value: 0x%x, Base: 0x%lx", parg2, (unsigned int) arg3, parg4);
			}
			break;
		...
		case USB_XHCI_TRACE(kTPXHCIWrite32Reg):
			{
				log (info, "XHCI", "Write32Reg", parg1, "Register: 0x%lx, Value: 0x%x, Base: 0x%lx", parg2, (unsigned int) arg3, parg4);
			}
			break;
