 ...
 - (void) DumpResults:(IrDAStatus *)stats
{
	...
	if (stats->connectionSpeed != oldStatus.connectionSpeed){
		...
		sprintf(temp, "%lu", (unsigned long)stats->connectionSpeed);
		[connectionSpeed setStringValue: [NSString stringWithUTF8String: temp]];
	}
	...
	if (stats->crcErrors != oldStatus.crcErrors){
		...
		sprintf(temp, "%lu", (unsigned long)stats->crcErrors);
		[crcErrors setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->dataPacketsIn != oldStatus.dataPacketsIn){
		oldStatus.dataPacketsIn = stats->dataPacketsIn;
		sprintf(temp, "%lu", (unsigned long)stats->dataPacketsIn);
		[dataPacketsIn setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->dataPacketsOut != oldStatus.dataPacketsOut){
		oldStatus.dataPacketsOut = stats->dataPacketsOut;
		sprintf(temp, "%lu", (unsigned long)stats->dataPacketsOut);
		[dataPacketsOut setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->dropped != oldStatus.dropped){
		oldStatus.dropped = stats->dropped;
		sprintf(temp, "%lu", (unsigned long)stats->dropped);
		[dropped setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->iFrameRec != oldStatus.iFrameRec){
		oldStatus.iFrameRec = stats->iFrameRec;
		sprintf(temp, "%lu", (unsigned long)stats->iFrameRec);
		[iFrameRec setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->iFrameSent != oldStatus.iFrameSent){
		oldStatus.iFrameSent = stats->iFrameSent;
		sprintf(temp, "%lu", (unsigned long)stats->iFrameSent);
		[iFrameSent setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->ioErrors != oldStatus.ioErrors){
		oldStatus.ioErrors = stats->ioErrors;
		sprintf(temp, "%lu", (unsigned long)stats->ioErrors);
		[ioErrors setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	[nickName setStringValue: [NSString stringWithUTF8String: (const char *)stats->nickName]];
	
	if (stats->protcolErrs != oldStatus.protcolErrs){
		makeSound = YES;
		oldStatus.protcolErrs = stats->protcolErrs;
		sprintf(temp, "%lu", (unsigned long)stats->protcolErrs);
		[protocolErrs setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->recTimeout != oldStatus.recTimeout){
		oldStatus.recTimeout = stats->recTimeout;
		sprintf(temp, "%lu", (unsigned long)stats->recTimeout);
		[recTimeout setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->rejRec != oldStatus.rejRec){
		oldStatus.rejRec = stats->rejRec;
		sprintf(temp, "%u", stats->rejRec); 
		[rejRec setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->rejSent != oldStatus.rejSent){
		oldStatus.rejSent = stats->rejSent;
		sprintf(temp, "%u", stats->rejSent); 
		[rejSent setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->resent != oldStatus.resent){
		oldStatus.resent = stats->resent;
		sprintf(temp, "%lu", (unsigned long)stats->resent);
		[resent setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->rnrRec != oldStatus.rnrRec){
		oldStatus.rnrRec = stats->rnrRec;
		sprintf(temp, "%u", stats->rnrRec); 
		[rnrRec setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->rnrSent != oldStatus.rnrSent){
		oldStatus.rnrSent = stats->rnrSent;
		sprintf(temp, "%u", stats->rnrSent); 
		[rnrSent setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->rrRec != oldStatus.rrRec){
		oldStatus.rrRec = stats->rrRec;
		sprintf(temp, "%lu", (unsigned long)stats->rrRec);
		[rrRec setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->rrSent != oldStatus.rrSent){
		oldStatus.rrSent = stats->rrSent;
		sprintf(temp, "%lu", (unsigned long)stats->rrSent);
		[rrSent setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->srejRec != oldStatus.srejRec){
		oldStatus.srejRec = stats->srejRec;
		sprintf(temp, "%u", stats->srejRec); 
		[srejRec setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->srejSent != oldStatus.srejSent){
		oldStatus.srejSent = stats->srejSent;
		sprintf(temp, "%u", stats->srejSent); 
		[srejSent setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->uFrameSent != oldStatus.uFrameSent){
		oldStatus.uFrameSent = stats->uFrameSent;
		sprintf(temp, "%lu", (unsigned long)stats->uFrameSent);
		[uFrameSent setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->uFrameRec != oldStatus.uFrameRec){
		oldStatus.uFrameRec = stats->uFrameRec;
		sprintf(temp, "%lu", (unsigned long)stats->uFrameRec);
		[uFrameRec setStringValue: [NSString stringWithUTF8String: temp]];
	}
	
	if (stats->xmitTimeout != oldStatus.xmitTimeout){
		oldStatus.xmitTimeout = stats->xmitTimeout;
		sprintf(temp, "%lu", (unsigned long)stats->xmitTimeout);
		[xmitTimeout setStringValue: [NSString stringWithUTF8String: temp]];
	}
...
- (IBAction)StartTimer:(id)sender
{
	if (state){
		...
		const char		*driverCStringName = [driverName UTF8String];
